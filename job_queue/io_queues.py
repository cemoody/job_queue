...
class IOQueues:
    ...
    def __init__(
        ...
        self.input_q = (
            SQLiteAckQueue(filename, table_name=input_q_name, **queue_kwargs)
            if input_q_name
            else None
        )
        self.output_q = (
            SQLiteAckQueue(filename, table_name=output_q_name, **queue_kwargs)
            if output_q_name
            else None
        )
        ...
        items = [
            {
                ...
                if c not in ["_id", "timestamp", "status"]
            }
            for row in rows
        ]
...
def test_ioq_gets(n=25):
    ...
    if os.path.exists(fn):
        os.remove(fn)
    ioq = IOQueues(
        "./test_cache", input_q_name="test_inputq", output_q_name="test_outputq"
    )
    assert ioq.size_ready() == 0
